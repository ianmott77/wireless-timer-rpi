...
void
Arduino::setType(int type)
{
    this->type = type;
}

void
Arduino::setNextAddress(int next)
{
    this->nextAddress = next;
}

void
Arduino::setPrevAddress(int prev)
{
    this->prevAddress = prev;
}

int
Arduino::getNextAddress()
{
    return this->nextAddress;
}

int
Arduino::getPrevAddress()
{
...
bool
Arduino::addDevice(int ID, int position, Device * device)
{
    if (ID != -1 && position != -1) {
        if (position > this->getPosition() + 1 || position < this->getPosition()) {
            emit error(205);
            return false;
        }
        if (this->manager->connect(I2C)) {
                    ...
                    if (this->manager->send(position)) {
                        ...
                        if (this->manager->connect(SERIAL)) {
                            ...
                            if (confirm == 1) {
                                ...
                                // error adding device to network, either the wrong device ID or it's to far away
...
Arduino::receiveIntervalSignal()
{
    ...
    std::cout << "monitor stopped" << std::endl << std::flush;
}
...
QQueue<Racer *> *
Arduino::getTimeQueue()
{
    return this->timeQueue;
}

Racer *
Arduino::getRacerInfo()
{
    Packet * p = 0;
    int bib    = 0;
...
void
Arduino::stopSerialLine()
{
    this->monitor->cancel();
}

std::thread *
Arduino::startSerialLine()
{
...
bool
Arduino::isMonitorRunning()
{
    return monitorRunning.load();
}

void
Arduino::setIntervalStatus(bool status)
{
...
void
Arduino::setFinishDistance(int distance)
{
...
int
Arduino::getFinishDistance()
{
    return this->distance;
}

void
Arduino::startDistanceMeasurement()
{
    this->dist->start();
}

void
Arduino::updateDistance(int distance)
{
    this->setDistance(distance);
}
...
void
Arduino::setDistance(int distance)
{
    this->distance = distance;
}

void
Arduino::startRacer(int bib)
{
