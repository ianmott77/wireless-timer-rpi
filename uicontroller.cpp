...
UIController::UIController(Arduino * arduino, QQmlApplicationEngine * engine, QObject * qml,
  QObject * parent) : QObject(parent)
{
    ...
    QObject::connect(this->arduino, SIGNAL(error(int)), this, SLOT(error(int)));
    this->qml = qml;
...
QString
UIController::getLoadingMsg()
{
    return this->loadingMsg;
}

void
UIController::error(int error)
{
...
void
UIController::startRacer(int bib)
{
...
void
UIController::setLoadingMsg(QString msg)
{
    this->loadingMsg = msg;
}
...
void
UIController::stop(QString reason)
{
...
bool
UIController::addDevice(int ID, int pos)
{
    emit this->busy(QString("Adding device"));
    this->tempDevice = new Device(ID, pos);
    while (this->arduino->getMonitor()->isRunning()) ;  // wait
    if (this->arduino->addDevice(this->tempDevice->getID(), this->tempDevice->getPosition())) {
        this->arduino->sync();
    }
    emit free();
}
...
void
UIController::choiceDispatch(int choice)
{
    Packet * p = 0;
            ...
            if (this->arduino->addDevice(-1, -1, device)) {
                ...
                if (this->arduino->getManager()->connect(SERIAL)) {
                    if (this->arduino->getManager()->send(1)) {
                        while (!this->arduino->getManager()->getCurrentConnection()->available()) ;  // wait
                        int confirm = 0;
 ...
 // check if we received a signal from the arduino either to add a new racer to the stack or that someone has crossed the finish
            if (this->arduino->getIntervalStatus()) {
                ...
                if (option == 1) { // somone crossed the finish line
                    ...
                    this->currentTime = (interval == 0) ? 0 : interval - finisher->getStartTime();
...
int
UIController::getCurrentBib()
{
    return this->bib;
}

int
UIController::getCurrentTime()
{
    return this->currentTime.load();
}

void
UIController::setCurrentBib(int bib)
{
    this->bib = bib;
}

void
UIController::startSerialLine()
{
...
int
UIController::getFinishDistance()
{
    return this->arduino->getFinishDistance();
}

void
UIController::setFinishDistance(int distance)
{
    this->arduino->setFinishDistance(distance);
}

void
UIController::startDistanceMeasurement()
{
    this->stopSerialLine();
...
void
UIController::updateDistance(int distance)
{
...
int
UIController::getDistance()
{
    return this->arduino->getDistance();
}

void
UIController::setDistance(int distance)
{
    this->arduino->setDistance(distance);
}

void
UIController::setQML(QObject * qml)
{
    this->qml = qml;
}
